...
void colorTemperatureLoop() {
  ...
  fill_rainbow( globalLedsArr + 5, NUM_LEDS - 5, --starthue, 20);

  // Choose which 'color temperature' profile to enable.
  uint8_t secs = (millis() / 1000) % (DISPLAYTIME * 2);
  if( secs < DISPLAYTIME) {
    FastLED.setTemperature( TEMPERATURE_1 ); // first temperature
    globalLedsArr[0] = TEMPERATURE_1; // show indicator pixel
  } else {
    FastLED.setTemperature( TEMPERATURE_2 ); // second temperature
    globalLedsArr[0] = TEMPERATURE_2; // show indicator pixel
  }

  // Black out the LEDs for a few secnds between color changes
  // to let the eyes and brains adjust
//  if( (secs % DISPLAYTIME) < BLACKTIME) {
//    memset8( globalLedsArr, 0, NUM_LEDS * sizeof(CRGB));
//  }
